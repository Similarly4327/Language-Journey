...
- `audio`: speaker registry (`ren`, `miku`) and local asset metadata. Assets use a stable `id`, registered `speakerId`, local `path`, and optional exact `textJa` to connect speakers to words or sentences; exercises and models may use their audio ID/key. Missing optional recordings produce a warning, not a broken lesson. Never call browser TTS at runtime as a substitute.
...
1. Plan new levels against the active Level 11–20 travel route. `templates/level-11.template.json` is only a template.
...
5. Check mobile UI, back navigation, saved progress and flashcard directions in a browser before release. Level 11–15 progress migrations must keep stable lesson IDs and map the former Level 11 supermarket exam rank to Level 15.
